...
static void nvme_suspend(rvvm_mmio_dev_t* dev, rvvm_state_t* state)
{
    nvme_dev_t* nvme = dev->data;
    if (!nvme) {
        return;
    ...
    rvvm_state_write_u32(state, atomic_load_uint32_relax(&nvme->conf));
    rvvm_state_write_u32(state, atomic_load_uint32_relax(&nvme->irq_mask));
    rvvm_state_write_u32(state, atomic_load_uint32_relax(&nvme->temp_thresh));
    rvvm_state_write(state, nvme->serial, sizeof(nvme->serial));

    for (size_t qid = 0; qid < STATIC_ARRAY_SIZE(nvme->sq); ++qid) {
        nvme_queue_t* q = &nvme->sq[qid];
        rvvm_state_write_u32(state, atomic_load_uint32_relax(&q->addr_l));
        rvvm_state_write_u32(state, atomic_load_uint32_relax(&q->addr_h));
        rvvm_state_write_u32(state, atomic_load_uint32_relax(&q->size));
        rvvm_state_write_u32(state, atomic_load_uint32_relax(&q->head));
        rvvm_state_write_u32(state, atomic_load_uint32_relax(&q->tail));
        rvvm_state_write_u32(state, atomic_load_uint32_relax(&q->data));
    }
    for (size_t qid = 0; qid < STATIC_ARRAY_SIZE(nvme->cq); ++qid) {
        nvme_queue_t* q = &nvme->cq[qid];
        rvvm_state_write_u32(state, atomic_load_uint32_relax(&q->addr_l));
        rvvm_state_write_u32(state, atomic_load_uint32_relax(&q->addr_h));
        rvvm_state_write_u32(state, atomic_load_uint32_relax(&q->size));
        rvvm_state_write_u32(state, atomic_load_uint32_relax(&q->head));
...
static void nvme_resume(rvvm_mmio_dev_t* dev, rvvm_state_t* state)
{
    nvme_dev_t* nvme = dev->data;
    if (!nvme) {
        return;
    ...
    uint32_t conf = 0;
    uint32_t irq_mask = 0;
    uint32_t temp_thresh = 0;
    if (!rvvm_state_read_u32(state, &conf) || !rvvm_state_read_u32(state, &irq_mask) || !rvvm_state_read_u32(state, &temp_thresh)
     || !rvvm_state_read(state, nvme->serial, sizeof(nvme->serial))) {
        rvvm_state_fail(state);
        return;
    }

    atomic_store_uint32_relax(&nvme->conf, conf);
    atomic_store_uint32_relax(&nvme->irq_mask, irq_mask);
    atomic_store_uint32_relax(&nvme->temp_thresh, temp_thresh);
    atomic_store_uint32_relax(&nvme->threads, 0);
    ...
    for (size_t qid = 0; qid < STATIC_ARRAY_SIZE(nvme->sq); ++qid) {
        ...
        uint32_t tail = 0;
        uint32_t data = 0;
        if (!rvvm_state_read_u32(state, &addr_l) || !rvvm_state_read_u32(state, &addr_h) || !rvvm_state_read_u32(state, &size)
         || !rvvm_state_read_u32(state, &head) || !rvvm_state_read_u32(state, &tail) || !rvvm_state_read_u32(state, &data)) {
            rvvm_state_fail(state);
            return;
        }
        atomic_store_uint32_relax(&q->addr_l, addr_l);
        atomic_store_uint32_relax(&q->addr_h, addr_h);
        atomic_store_uint32_relax(&q->size, size);
        atomic_store_uint32_relax(&q->head, head);
        atomic_store_uint32_relax(&q->tail, tail);
        atomic_store_uint32_relax(&q->data, data);
    }
